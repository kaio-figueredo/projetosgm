...
<main class="main-content">
    <div class="w-100" style="max-width: 600px;">
        ...
        <div class="form-card">
            <div class="mb-4">
                <h3 class="fw-bold m-0">Novo Ambiente</h3>
                <p class="text-muted small">Vincule um novo espaço a um bloco existente</p>
            </div>
            ...
            <form id="formNovoAmbiente">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" id="nome_ambiente" class="form-control" placeholder="Ex: Laboratório de Informática 02" required>
                </div>

                <button type="submit" class="btn btn-submit w-100">
                    <i class="bi bi-plus-circle-fill me-2"></i> Cadastrar Ambiente
                </button>
            </form>
        </div>
    </div>
        ...
        try {
            ...
            if (blocos && blocos.length > 0) {
                blocos.forEach(bloco => {
                    ...
                    option.value = parseInt(bloco.id_bloco);
                    option.textContent = bloco.nome;
                    select.appendChild(option);
                });
            } else {
                select.innerHTML = '<option value="">Nenhum bloco encontrado</option>';
            }
        } catch (error) {
            console.error("Erro ao carregar:", error);
            select.innerHTML = '<option value="">Erro ao conectar com a API</option>';
        }
    }

    // Envio do formulário para api/ambiente.php (Método POST)
    document.getElementById('formNovoAmbiente').addEventListener('submit', async (e) => {
        e.preventDefault();

        const dados = {
            id_bloco: document.getElementById('selectBloco').value,
            nome: document.getElementById('nome_ambiente').value
        };
