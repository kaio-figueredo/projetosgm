...
switch($method){
    ...
    $desc = $conn->real_escape_string(trim($data->descricao));
       ...
       case 'DELETE':
        ...
        // Verifica se o JSON é válido e se o ID existe
        if (!$data || !isset($data->id_tipo)) {
            echo json_encode(["success" => false, "message" => "ID do tipo de serviço não fornecido ou JSON inválido."]);
            exit;
        }

        $id_tipo = (int)$data->id_tipo;

        // Uso de Prepared Statement para maior segurança
        ...
        try {
            if ($stmt->execute()) {
                if ($stmt->affected_rows > 0) {
                    ...
                    echo json_encode(["success" => false, "message" => "Nenhum registro encontrado com esse ID."]);
                }
            } else {
                throw new Exception($stmt->error);
            }
        } catch (Exception $e) {
            // Caso haja erro de chave estrangeira (registros vinculados)
            echo json_encode([
                "success" => false, 
                "message" => "Erro ao excluir: este item pode estar sendo usado em outros registros."
            ]);
        }
